...
from helper.loops import train_vanilla as train1, train_fer as train2, validate #train1 is STD, Train2 is FER. Need to get accuracy from Train1 and compare to accuracy from Train2
...
def main():
    ...
    train_soft = torch.zeros((n_sample,n_cls), dtype=torch.float).cuda()
    
    train_index = torch.zeros(n_sample)
    
    train_index = train_index.type(torch.ByteTensor).cuda()
    
    train_probs = torch.zeros(n_sample)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if epoch<=opt.init: #comment for STD/ Uncomment for FER
        ...
        else: #comment for STD/ uncomment for FER
            train_acc, train_soft, train_index = train2(epoch, train_loader, model, criterion_list, optimizer, opt, train_soft, train_index) #comment for STD/ uncomment for FER
